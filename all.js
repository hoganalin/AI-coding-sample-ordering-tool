...
const CONFIG = {
  ...
  // 請填入您的 GCP API Key
  API_KEY: "AIzaSyDP5oMErX13FCNCkcnQImF4X-TioQyhY1M",
  // 請填入您的 Google Sheet ID
  SPREADSHEET_ID: "1fwkyUxV6tfT5YdiYnD9sF3jc1wHkgz5S9Am4IvBBYyE",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("click", showConfigPanel);
  document
    .getElementById("btn-save-config")
    .addEventListener("click", saveTodayConfig);
  document
    .getElementById("btn-clear-orders")
    .addEventListener("click", clearOrders);
  document
    .getElementById("btn-show-orders")
    .addEventListener("click", openOrdersModal);
  document
    .querySelector(".close-modal")
    .addEventListener("click", closeOrdersModal);
  document
    .getElementById("btn-copy-orders")
    .addEventListener("click", copyOrdersToClipboard);
...
function renderMenu(todayRestaurants, allMenu) {
  ...
  const todayMenu = allMenu.filter((item) =>
    todayRestaurants.includes(item.restaurant),
  );
